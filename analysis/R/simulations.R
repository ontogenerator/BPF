...
# probe_reversal <- flex_res %>% 
#   filter(n_poke > 51, n_poke < 202) %>% 
#   group_by(ind, prob, l_rate, temperature, phase) %>% 
#   summarise(n_rewarding = sum(mask),
#             n_new_rewarding = sum(mask2)) 
# 
# probe_reversal %>% 
#   ggplot() +
#   stat_summary(aes(prob, n_rewarding, color = as.factor(l_rate), group = l_rate)) +
#   facet_grid(phase ~ temperature, labeller = label_both) +
#   labs(x = "reward probability", y = "visits to rewarding flowers",
#        color = "learning rate")



#asymptotic performance achieved in about 100 visits (taking above 200 is definitely sufficient)
...
#   stat_summary(size = 0.7) +
#   facet_grid(. ~ l_rate, labeller = label_both) +
#   labs(title = "Effect of learning rate and temperature on sampling at different probabilities",
#        x = "probability", color = "temperature")
# block_size <- 10
# 
# binned_mem <- flex_res %>%
#   filter(!is.na(mask)) %>%
#   group_by(prob, ind, l_rate, temperature, cond) %>%
#   mutate(n_poke = n_poke - 1,
#          vis_bins = cut(n_poke, breaks = seq(0, 500, block_size))) %>%
#   group_by(prob, ind, l_rate, temperature, vis_bins, cond) %>%
#   summarise(n_rewarding = sum(mask),
#             n_new_rewarding = sum(mask2),
#             n_total = n()) %>%
...
#   geom_line(aes(as.numeric(vis_bins), n_rewarding, color = as.factor(l_rate), group = cond)) +
#   geom_line(aes(as.numeric(vis_bins), n_new_rewarding, color = as.factor(l_rate), group = cond), linetype = 2) +
#   facet_grid(prob ~ temperature, labeller = label_both) +
#   labs(title = "Learning curves as a function of temperature, reward probability, and learning rate",
#        x = paste("Block of", block_size, "visits"), y = "visits to rewarding flowers",
#        color = "learning rate") +
#   geom_vline(xintercept = c(10, 15), linetype = 2) +
#   annotate(geom = "text", label = "phase 1", x = 5, y = 4) +
#   annotate(geom = "text", label = "ph.2", x = 12.5, y = 10) +
#   annotate(geom = "text", label = "phase 3", x = 25, y = 4) +
#   geom_hline(yintercept = 2/12*block_size, linetype = 3) +
#   annotate(geom = "text", label = "chance level = 2/12", x = 40, y = 2.5)
# 
# 
# binned_mem %>%
#   filter(ind == 20, l_rate < 0.5) %>%
#   ggplot() +
#   geom_line(aes(as.numeric(vis_bins), n_rewarding, color = as.factor(l_rate), group = cond)) +
#   geom_line(aes(as.numeric(vis_bins), n_new_rewarding, color = as.factor(l_rate), group = cond), linetype = 2) +
#   facet_grid(prob ~ temperature, labeller = label_both) +
